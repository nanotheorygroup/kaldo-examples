...
The example `carbon_diamond_Tersoff_ASE_LAMMPS` illustrates how to perform thermal transport simulations for a carbon 
diamond (2 atoms per cell) system using [ASE and LAMMPS](https://wiki.fysik.dtu.dk/ase/_modules/ase/calculators/lammpslib.html) as a force calculator.
...
-   `carbon_diamond_Tersoff_thermal_conductivity.py` proceeds as follows:
    ...
    1. Set up the force constant object and compute the 2nd and 3rd force constants using `C.tersoff`.

    2. Set up the phonon object (15x15x15 k-point mesh) and perform the quantum simulation at 300K.
