...
1. Preliminary step: Perform vc-relax in QE to relax the structures. After finishing the calculation of vc-relax, run `uv run python export-POSCAR.py` (requires [ASE](https://wiki.fysik.dtu.dk/ase/)) to generate `POSCAR`. 
...
5. Perform BTE calculations using [kALDo](https://github.com/nanotheorygroup/kaldo) in the `05-kALDo_runs_BTE` folder. Like in the previous step, create a directory `fc_DFT` and copy `POSCAR`, `espresso.ifc2` and `FORCE_CONSTANTS_3RD` into it. Then run `uv run python thermal_conductivity.py` to calculate. See `thermal_conductivity.py` for a detailed description. Here is a script to describe this step: 
