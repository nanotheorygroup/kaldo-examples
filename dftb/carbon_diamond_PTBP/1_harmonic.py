...
# Computes: 2nd order force constants and harmonic properties for carbon diamond (2 atoms per cell)
# Uses: ASE, DFTB+
...
print('Frequencies at Gamma (THz):')
for i, f in enumerate(phonons.frequency[0]):
    print('  Mode %d: %.2f' % (i, f))

freq = phonons.frequency.flatten()
print('\nFrequency range: %.2f - %.2f THz' % (freq[freq > 0].min(), freq.max()))
